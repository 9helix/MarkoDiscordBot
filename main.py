...
def sun_find():
    ...
    embed = discord.Embed(
        title="Solar Activity", description=desc, color=discord.Colour.orange()
    )
...
def moon_find():
    ...
    embed = discord.Embed(
        title="Moon's Phase", description=data, color=discord.Colour.blue()
    )
        ...
        for day in list(follow_dict[time]):
            ...
            for serie in list(follow_dict[time][day]):
                ...
                if show.start != time:
                    ...
                    follow_dict.setdefault(show.start, {}).setdefault(
                        show.weekday, {}
                    ).setdefault(show.name, [show.cur_episodes, show.max_episodes, x])[
                        2
                    ]
                    continue
                try:
...
def detect(code):
    ...
    if "myanimelist.net" in code:
        ...
        code = [
            x.lower()
            for x in code.translate(str.maketrans("", "", string.punctuation)).split()
        ]
...
class mirko(discord.Client):
    ...
    async def setup_hook(self) -> None:
        ...
        if pkl_read("follow_dict") != {}:
            self.anime_follow.change_interval(time=[x for x in pkl_read("follow_dict")])
            self.anime_follow.start()
            print("anime_follow started since its not empty", pkl_read("follow_dict"))
    ...
    async def ch_pr(self):
        ...
        act = (
            ...
            if statuses.index(status) < 2
                ...
                else discord.Activity(type=discord.ActivityType.listening, name=status)
            )
        )
        ...
        if cur_weekday in follow_dict[now]:  # time je bio prije umjesto now
            ...
            for anim in list(follow_dict[now][cur_weekday]):
                for user in follow_dict[now][cur_weekday][anim][2]:
                    ...
                    print(user)
                    await user.send(
                        f"**{anim}** episode **{follow_dict[now][cur_weekday][anim][0]+1}** has been released!"
                    )
                if (
                    follow_dict[now][cur_weekday][anim][1] != "?"
                    and follow_dict[now][cur_weekday][anim][0] + 1
                    == follow_dict[now][cur_weekday][anim][1]
                ):
            ...
            for anim in follow_dict[now][day]:
                for user in follow_dict[now][day][anim][2]:
                    ...
                    print(user)
                    await user.send(
                        f"**{anim}** episode **{follow_dict[now][day][anim][0]+1}** has been released!"
                    )
                if (
                    follow_dict[now][day][anim][1] != "?"
                    and follow_dict[now][day][anim][0] + 1
                    == follow_dict[now][day][anim][1]
                ):
...
@tree.command(
    name="anime_list",
    description="Sends names and MAL URLs for known anime for easier search.",
)
...
@tree.command(
    name="anime",
    description="Sends data about given anime. Use code, MAL URL or anime name to get data.",
)
async def self(interaction: discord.Interaction, code: str):
    ...
    if show.success:
        ...
        out = discord.Embed(
            title=show.name, description=show.__str__(), color=genres[show.genre1]
        )
...
async def self(interaction: discord.Interaction, code: str, user: discord.User = None):
    ...
    if user:
        if interaction.user.id != admin:
            ...
            await interaction.response.send_message(
                f"Only admin can follow anime for another person."
            )
            return
        else:
    ...
    if show.success:
        ...
        if "Currently Airing 🟢" == show.status:
            ...
            follow_dict.setdefault(show.start, {}).setdefault(
                show.weekday, {}
            ).setdefault(show.name, [show.cur_episodes, show.max_episodes, []])[2]

            if (
                interaction.user.id
                not in follow_dict[show.start][show.weekday][show.name][2]
            ):
            ...
            if show.broadcast == "Unknown":
                ...
                if (
                    interaction.user.id
                    not in follow_dict[show.start][show.airstart][show.name][2]
                ):
    ...
    if show.success:
        ...
        if "Currently Airing 🟢" == show.status:
            ...
            try:
                ...
                pkl_write("follow_dict", follow_dict)
                print(f"Successfully unsubscribed from {show.name}.")
                await interaction.followup.send(
                ...
                await interaction.followup.send(
                    f"You are not subscribed to **{show.name}**."
                )
...
@tree.command(
    name="follow_list",
    description="Sends list of followed animes and users that have followed them.",
)
async def self(interaction: discord.Interaction):
    msg = ""
    with open("database/follow_dict.pkl", "rb") as f:
    ...
    if follows:
        for item in follows:
            for item2 in follows[item]:
                for anim in follows[item][item2]:
                    users = list(map(bot.get_user, follows[item][item2][anim][2]))
                    string = ""
                    for i in users:
        ...
        msg = (
            "Follow list is currently empty. Use `/follow` command to add anime to it."
        )
    await interaction.response.send_message(msg)
...
@tree.command(
    name="delay_anime",
    description="Delays anime by 1 week. Use code, MAL URL or anime name to get data.",
)
async def self(interaction: discord.Interaction, code: str):
    await interaction.response.defer(ephemeral=False)
    code, err = detect(code)
...
@tree.command(
    name="unblock", description="Unblocks a user from using the bot [DEV ONLY!]"
)
async def self(interaction: discord.Interaction, user: discord.Member):
    if interaction.user.id in devs:
        try:
...
@tree.command(
    name="dm", description="DMs a given user/sender with a custom/random message"
)
async def self(
    interaction: discord.Interaction, user: discord.User = None, message: str = None
):
...
@tree.command(
    name="dev_add", description="Marks a specific user as a bot's dev [DEV ONLY!]"
)
...
@tree.command(
    name="dev_remove", description="Removes a specific user as a bot's dev [DEV ONLY!]"
)
async def self(interaction: discord.Interaction, user: discord.User):
    if interaction.user.id in devs:
        try:
